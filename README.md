...
included in each commit. But Grex provides a way to automate this.
...
Grex will split and squash the first set of commits into the second. It will squash together commits that modify the
same files. This doesn't work if multiple commits in the second set modify the same file, as Grex doesn't know which
...
`grex-editor`. You can now run git rebases with the grex editor.
